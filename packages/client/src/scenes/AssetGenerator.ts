...
/* =============================================
 * ENHANCED CHARACTER SPRITES — 48x48 chibi
 * ============================================= */
function generatePlayerSprites(scene: Phaser.Scene) {
  createCharacterTexture(scene, "player-left", COLORS.teamLeft, 0x5aaa8a, "left");
  createCharacterTexture(scene, "player-right", COLORS.teamRight, 0xd48a9a, "right");
...
function createCharacterTexture(
  ...
  const bodyStr = rgbStr(br, bg, bb);
  const darkStr = rgbStr(dr, dg, db);
  const lightStr = rgbStr(
    Math.min(255, br + 50),
    Math.min(255, bg + 50),
    Math.min(255, bb + 50)
  );
  const shadowStr = rgbStr(
    Math.max(0, br - 50),
    Math.max(0, bg - 50),
    Math.max(0, bb - 50)
  );

  // Outfit colors (team-colored shirt)
  const shirtColor = team === "left" ? "#4a9a7a" : "#c46a7a";
  const shirtDark = team === "left" ? "#3a7a5a" : "#a45060";
  const hatColor = team === "left" ? "#3a8a6a" : "#b4506a";
  const hatBand = team === "left" ? "#ffd93d" : "#ffd93d";
  const shoeColor = team === "left" ? "#4a6a5a" : "#8a4a5a";

  // --- Drop shadow ---
  ctx.fillStyle = "rgba(0,0,0,0.2)";
  ctx.beginPath();
  ctx.ellipse(cx, cy + 19, 13, 5, 0, 0, Math.PI * 2);
  ctx.fill();

  // --- Small legs ---
  ctx.fillStyle = shadowStr;
  ctx.fillRect(cx - 7, cy + 11, 5, 7);
  ctx.fillRect(cx + 2, cy + 11, 5, 7);
  // Shoes with rounded tops
  ctx.fillStyle = shoeColor;
  ctx.beginPath();
  ctx.ellipse(cx - 5, cy + 17, 4, 3, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(cx + 5, cy + 17, 4, 3, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.beginPath(); ctx.arc(cx + 4, cy + 16, 1.5, 0, Math.PI * 2); ctx.fill();

  // --- Body (torso with team shirt) ---
  ...
  // Belly highlight (shirt fabric sheen)
  ctx.fillStyle = "rgba(255,255,255,0.18)";
  ctx.beginPath();
  ctx.ellipse(cx - 2, cy + 3, 6, 5, 0, 0, Math.PI * 2);
  ctx.fill();

  // Belt line
  ctx.strokeStyle = shadowStr;
  ctx.lineWidth = 1;
  ctx.globalAlpha = 0.3;
  ctx.beginPath();
  ctx.ellipse(cx, cy + 10, 10, 3, 0, 0, Math.PI);
  ctx.stroke();
  ctx.globalAlpha = 1;
  ...
  // Left arm
  ctx.beginPath();
  ctx.ellipse(cx - 14, cy + 3, 4, 7, 0.3, 0, Math.PI * 2);
  ctx.fill();
  // Right arm
  ctx.beginPath();
  ctx.ellipse(cx + 14, cy + 3, 4, 7, -0.3, 0, Math.PI * 2);
  ...
  ctx.globalAlpha = 0.4;
  ctx.beginPath();
  ctx.ellipse(cx - 14, cy + 5, 3, 4, 0.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(cx + 14, cy + 5, 3, 4, -0.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
  // Hands (skin-colored round tips)
  const skinColor = "#f5d0b0";
  ctx.fillStyle = skinColor;
  ctx.beginPath(); ctx.arc(cx - 14, cy + 9, 3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + 14, cy + 9, 3, 0, Math.PI * 2); ctx.fill();

  // --- Head (large, chibi ~60% of height) ---
  // Head shadow behind
  ctx.fillStyle = "rgba(0,0,0,0.1)";
  ctx.beginPath();
  ctx.arc(cx + 1, cy - 5, 16, 0, Math.PI * 2);
  ctx.fill();

  // Main head (skin-toned base)
  ctx.fillStyle = skinColor;
  ctx.beginPath();
  ctx.arc(cx, cy - 7, 15, 0, Math.PI * 2);
  ctx.fill();

  // Head top-light radial gradient
  const headGrad = ctx.createRadialGradient(cx - 4, cy - 14, 2, cx, cy - 7, 15);
  headGrad.addColorStop(0, "rgba(255,240,220,0.6)");
  ...
  // Bottom of head shading
  const headShadow = ctx.createRadialGradient(cx, cy + 4, 2, cx, cy - 7, 15);
  headShadow.addColorStop(0, "rgba(180,130,100,0.15)");
  headShadow.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = headShadow;
  ctx.beginPath();
  ctx.arc(cx, cy - 7, 15, 0, Math.PI * 2);
  ctx.fill();

  // --- Hat (team-colored beanie / cap) ---
  ctx.fillStyle = hatColor;
  ctx.beginPath();
  ctx.arc(cx, cy - 7, 15, Math.PI, 2 * Math.PI);
  ctx.fill();
  // Hat brim shadow
  ctx.fillStyle = "rgba(0,0,0,0.1)";
  ...
  ctx.beginPath();
  ctx.ellipse(cx - 4, cy - 17, 6, 3, -0.2, 0, Math.PI * 2);
  ctx.fill();
  // Hat pom-pom / top detail
  ctx.fillStyle = hatBand;
  ctx.beginPath(); ctx.arc(cx, cy - 22, 3, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "rgba(255,255,255,0.3)";
  ctx.beginPath(); ctx.arc(cx - 1, cy - 23, 1.2, 0, Math.PI * 2); ctx.fill();

  // --- Eyes (big, cute, with colored iris) ---
  // Eye whites
  ctx.fillStyle = "#ffffff";
  ctx.beginPath(); ctx.ellipse(cx - 6, cy - 6, 5, 5.5, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx + 6, cy - 6, 5, 5.5, 0, 0, Math.PI * 2); ctx.fill();

  // Colored iris (team-tinted)
  const irisColor = team === "left" ? "#3a7a6a" : "#9a4a5a";
  ctx.fillStyle = irisColor;
  ctx.beginPath(); ctx.arc(cx - 5, cy - 5, 3.5, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + 7, cy - 5, 3.5, 0, Math.PI * 2); ctx.fill();
  ...
  // Eye highlights — double dots for sparkle
  ctx.fillStyle = "#ffffff";
  ctx.beginPath(); ctx.arc(cx - 6, cy - 7, 2, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + 6, cy - 7, 2, 0, Math.PI * 2); ctx.fill();
  // Second smaller highlight
  ctx.beginPath(); ctx.arc(cx - 4, cy - 4, 0.9, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + 8, cy - 4, 0.9, 0, Math.PI * 2); ctx.fill();

  // --- Rosy cheeks ---
  ctx.fillStyle = "rgba(255, 110, 110, 0.3)";
  ctx.beginPath(); ctx.ellipse(cx - 11, cy - 2, 4, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(cx + 11, cy - 2, 4, 3, 0, 0, Math.PI * 2); ctx.fill();
  ...
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.arc(cx, cy - 1, 4, 0.15 * Math.PI, 0.85 * Math.PI, false);
  ctx.stroke();

  // Small nose dot
  ctx.fillStyle = "rgba(180,130,100,0.3)";
  ctx.beginPath(); ctx.arc(cx + 1, cy - 3, 1, 0, Math.PI * 2); ctx.fill();

  // --- 1px black outline for pixel art definition ---
  ctx.strokeStyle = "rgba(20,20,20,0.45)";
  ctx.lineWidth = 1;
  // Head outline
  ctx.beginPath(); ctx.arc(cx, cy - 7, 15.5, 0, Math.PI * 2); ctx.stroke();
  // Body outline
  ctx.beginPath(); ctx.ellipse(cx, cy + 5, 12.5, 10.5, 0, 0, Math.PI * 2); ctx.stroke();
...
function createDeadTexture(scene: Phaser.Scene, key: string) {
  ...
  ctx.quadraticCurveTo(cx - 10, cy + 8, cx - 5, cy + 12);
  ctx.quadraticCurveTo(cx, cy + 16, cx + 5, cy + 12);
  ctx.quadraticCurveTo(cx + 10, cy + 8, cx + 14, cy + 12);
  ...
  // Swirl eyes
  ctx.strokeStyle = "rgba(100, 100, 120, 0.7)";
  ctx.lineWidth = 2;
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.arc(cx - 6, cy - 5, 4, 0, Math.PI * 1.5);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(cx - 6, cy - 5, 2, Math.PI, Math.PI * 2.5);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(cx + 6, cy - 5, 4, 0, Math.PI * 1.5);
  ctx.stroke();
